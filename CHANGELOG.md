...
- **BREAKING: `Provider` behaviour reshaped to two callbacks; the
  ...
  request (a private `{url, json}`) and executes it via the shared
  `Provider.request/4`. Payload tests assert on the wire through a
  `Req.Test` plug (`captured_request/2`), not a public builder.
  `Provider.resolve_http_client/2` and the `:http_client` opts convention
  it fished from are gone — `infer` takes the `Client` struct and reads
  the prebuilt transport off it, so a missing transport is no longer
  ...
  the 0.8.0 freeze): with no known third-party implementors it may be
  reshaped in minor releases with changelog notice — exactly what this
  cycle did — while `Provider.Response` stays Core and the
  `t:Provider.error/0` union stays contract (it is what
  `ChunkError.reason` consumers match on), evolving additively.
