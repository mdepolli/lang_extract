defmodule LangExtract.ChunkerDifferentialTest do
  ...
  @words ~w(alpha Beta gamma DELTA the quick brown fox jumps 42 3.14 café 東京 Dr Mr Prof)
  @terminators [".", "!", "?", "。", "...", "?!"]
  @closers [~s("), "'", ")", "]", "”", "’", ~s|")|, "’”"]
  @separators [" ", "  ", "\n", "\r\n", "\n\n", " \n ", "\r", "\t"]
  @budgets [17, 64, 250, 1000]
  ...
  test "targeted shapes match the baseline" do
    boundary_free = Enum.map_join(1..20_000, " ", fn _ -> "word" end)
    ...
    for text <- [boundary_free, one_giant_token, terminator_wall, closer_wall, crlf_prose, ""],
        budget <- @budgets do
      assert Chunker.chunk(text, max_chunk_chars: budget) ==
               ChunkerBaseline.chunk(text, max_chunk_chars: budget)
